...
# Initialize model (Two link manipulator using the distributed mass matrix)
robot = TwoLinkArm(method='distributed')

# Initialize robot states (q1, q2, q1_dot, q2_dot)
...
# Data logging
history = {'q1':[], 'q2':[], 'q1_dot':[], 'q2_dot':[], 'x':[],
            'xdot':[], 'y':[], 'ydot':[], 'x_axis_pitch':[], 'y_axis_pitch':[]}
...
for t in time_steps:
    ...
    # Limit cycles pitch
    x_axis_pitch = x_axis_controller.get_pitch([x, xdot])
    y_axis_pitch = y_axis_controller.get_pitch([y, ydot])
    
    # get VER output for x-axis and  y-axis 
    u_ver_x = x_axis_controller.get_desired_acceleration([x, xdot], y_axis_pitch, phase_offsets[0])
    u_ver_y = y_axis_controller.get_desired_acceleration([y, ydot], x_axis_pitch, phase_offsets[1])
